...
phys_labels <- c("water_level" = "Water level",
...
chem <- chemicalvars_phytos |> 
  ...
  mutate(names = recode(names, !!!chem_labels)) |>
...
phys_phyto_only_drivers <- phyto_only  |> 
  select(matches("^(water|transparency|turb|light|tss).*(?:_vars|_drive)$"), res_name, name_yr) |> select(-c(res_name, name_yr, watercolor_vars, watercolor_drive)) |> 
...
phys <- physicalvars_phytos |> 
  select(names, notid, id, prop_id) |> 
  mutate(names = recode(names, !!!phys_labels)) |>
mutate(names = factor(names, levels = unique(names))) |>
  arrange(desc(prop_id)) |> 
  mutate(names = factor(names, levels = names)) |> 
  select(-prop_id) |> 
...
mutate(total_wl = ifelse(water_level_drive == 1 & water_level_vars == 0, 1, water_level_vars), 
       water_level_vars = total_wl) |> 
  select(-c(total_wl, watercolor_drive, watercolor_vars))
...
(exp(coef(logregform_cyano)*(-0.1)))

exp(coef(logregform_phyto)*(-0.1))
exp(coef(logregform_phyto)*(0.1))

exp(cbind(OR = coef(logregform_phyto), confint(logregform_phyto)))

exp(cbind(OR = coef(logregform_cyano), confint(logregform_cyano)))
...
phys_labels <- c("water_level" = "Water level",
                 ...
                 "flush_discharge" = "Flush & Discharge", 
                 "inflow" = "Inflow", 
...
phys_phyto_cyano_drivers <- phyto_cyano |>
  select(matches("^(water_level|water_temp|transparency|turb|light|tss|tds|strat|restime|flush_discharge|inflow|precip|air_temp).*(?:_vars|_drive)$"), res_name, name_yr) |>
  select(-c(res_name, name_yr)) |>
  mutate(total_wl = ifelse(water_level_drive == 1 & water_level_vars == 0, 1, water_level_vars),
         water_level_vars = total_wl) |>
  select(-total_wl)
...
ggsave(paste0(here(), "/Supplement_figures/FigS2_drivers_all_WL.jpg"), plot = combined, height = 6, width = 9, units = "in")
```
...
boxplotdata2_opposite <- boxplotdata2 %>% 
  ...
  #phytoplankton reverse response
  ...
  #cyanobacteria reverse response
  ...
  select(phyto_binary_decrease, phyto_binary_increase, cyano_binary_decrease, cyano_binary_increase, prop_1delta_wl_decrease, prop_1delta_wl_increase, phyto_binary_increase, phyto_binary_decrease, phyto_response_wl_up, phyto_response_wl_down, name_yr, trophic_status_reduced, prop_alldelta_wl_decrease, prop_alldelta_wl_increase, cyano_response_wl_up, cyano_response_wl_down)
...
phytoincrease_logreg <- boxplotdata2_opposite |> 
  ...
  filter(!is.na(phyto_binary_increase)) |> 
...
phytodecrease_logreg <- boxplotdata2_opposite |> 
  ...
  filter(!is.na(phyto_binary_decrease)) |> 
  filter(!is.na(deltawl))

phyto_logreg_reverse <- rbind(phytodecrease_logreg, phytoincrease_logreg)|> 
  select(type, response, deltawl, name_yr)
...
cyano_logreg_reverse <- rbind(cyanodecrease_logreg, cyanoincrease_logreg) |> 
  select(type, response, deltawl, name_yr)

all_logreg <- rbind(phyto_logreg_reverse, cyano_logreg_reverse)
...
logregform_phyto <-  glm(formula=response ~ deltawl, data=phyto_logreg_reverse, family=binomial)
summary(logregform_phyto)

logregform_cyano <-  glm(formula=response ~ deltawl, data=cyano_logreg_reverse, family=binomial)
summary(logregform_cyano)
...
boxplotdata2_opposite <- boxplotdata2 %>% 
    mutate(phyto_binary_increase = ifelse(phyto_response_wl_up == "Not significant", NA, 
                                ifelse(phyto_response_wl_up == "Decrease", 0, 
                                ifelse(phyto_response_wl_up == "Increase", 1, NA)))) %>% 
  mutate(phyto_binary_decrease = ifelse(phyto_response_wl_down == "Not significant", NA, 
                                ifelse(phyto_response_wl_down == "Decrease", 0, 
                                ifelse(phyto_response_wl_down == "Increase", 1, NA)))) %>%
   mutate(cyano_binary_increase = ifelse(cyano_response_wl_up == "Not significant", NA, 
                                ifelse(cyano_response_wl_up == "Decrease", 0, 
                                ifelse(cyano_response_wl_up == "Increase", 1, NA)))) %>% 
  mutate(cyano_binary_decrease = ifelse(cyano_response_wl_down == "Not significant", NA, 
                                ifelse(cyano_response_wl_down == "Decrease", 0, 
                                ifelse(cyano_response_wl_down == "Increase", 1, NA))))
...
all_logreg <- logreg_wldecrease/logreg_wlincrease/all_reverse/all_noNA + 
  ...
  plot_layout(axes = "collect_y",
              guides = "collect")
all_logreg
ggsave(paste0(here(),"/Supplement_figures/FigS1_logreg_all_supp.jpg"), plot = all_logreg, height = 8.5, width = 8, units = "in")
